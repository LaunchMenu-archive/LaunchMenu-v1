...
//    };
    ...
    //object class
    window.SyncedObject = class SyncedObject{
        constructor(){
            this[targetKey] = this;
        }
        init(name, parent, data){ //basically the constructor, but called after setting up the proxy
        ...
        setProperty(property, value, objectInitialisation){
            ...
            if(isExternal || objectInitialisation || $EventHandler.trigger("setObjectProperty:pre", $Settings, eventData)){                
                ...
                if(!objectInitialisation){                    
                    ...
                    parentSetting.__invokeListeners(2, ["set", this.getPath(false, true)+"."+property, this[property], oldValue]);
...
//                createObject: sendCreateObject,
        ...
        static createObjectProxy(syncedObject){
            return proxy(syncedObject);
        }
        static isValidObject(object){
            return (object instanceof Object) && !(object instanceof Function) && !(object instanceof RegExp);
        }
        static createProxiedSyncedObject(name, parent, object){
            var syncedObject = new SyncedObject();
            var proxy = this.createObjectProxy(syncedObject);
            //only initialize after the object contains a proxyKey, as some methods depend on it
            if(name)
                syncedObject.init(name, parent, object);
            return proxy;
//            return this.createObjectProxy(new SyncedObject(name, parent, object));
        }
    }
    
    //create proxy to forward any object changes
    const proxy = function(syncedObject){
        var p = new Proxy(syncedObject, {
