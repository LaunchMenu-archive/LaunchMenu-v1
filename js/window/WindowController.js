...
(function(){    
    ...
    WindowController = class WindowController{
        constructor(windowClassPath, args){
            this.__initVars();
            this.__loadScreenBounds();
            this.childID = highestChildWindowID++;
            this.arguments = args;
            ...
            if(typeof windowClassPath == "string"){ //get script from file
                ...
                if(script){
                    var path = script.absolutePath;
                    script = script.code;
                    
                    this.__createWindow(script, null, path);
                }else{
        ...
        __initVars(){
            ...
            this.debug = true; //open the debug window when the window opens
            this.loaded = false;
        }
        ...
        __onResize(newWidth, newHeight, oldWidth, oldHeight){}      //fires when the window resizes
        
        //window setup functions
        __createWindow(script, loadScriptsScript, absoluteSrcPath){
            //create the window
            var t = this;
        ...
        __receiveSetFullScreen(val, loc, size){
            if(val){
                ...
                this.window.setSize(Math.floor(this.screenBounds.width), Math.floor(this.screenBounds.height));
                this.window.setPosition(Math.floor(this.screenBounds.minX), Math.floor(this.screenBounds.minY));
                this.fullScreen = true;
                return oldPos;
            }else{
                this.__onResize(size[0], size[1], this.prevSize[0],this.prevSize[1]);
                this.window.setSize(Math.floor(size[0]), Math.floor(size[1]));
                this.window.setPosition(Math.floor(loc[0]), Math.floor(loc[1]));
        ...
        __setupFrameEvents(){
            ...
            this.realIpc.on("frameResize"+this.childID, function(event, data){
                t.resize(data.width, data.height);
                if(!windowVisible){ //show window once initial size has been determined
//                    setTimeout(function(){//timeout because otherwise the resize might not have been processed yet                        
...
//                    },20);
            ...
            this.realIpc.on("frameClose"+this.childID, function(event, data){
                t.close();
            });
            this.realIpc.on("frameMinimize"+this.childID, function(event, data){
                t.minimize();
            });
        }
        __setupIPC(){
            ...
            this.realIpc.on("setupWindowControllerIPC"+this.childID, function(event, windowFuncs){
                            ...
                            t[n] = function(){
                                ...
                                for(var i=0; i<ar.length; i++){
                                    ...
                                    if(typeof ar[i] == "function"){
                                        (function(f){
                                            t.realIpc.once(ipcReturnMessagePrefix+m[2][0].toUpperCase()+m[2].substring(1)+t.childID,function(event, data){
                                                f.call(t, data);
                                            });                                            
                                        })(ar[i]);
                                        ar.splice(i, 1);
            ...
            this.ipc = {
                on: function(channel, func){
                    return t.realIpc.on(ipcMessagePrefix+channel[0].toUpperCase()+channel.substring(1)+t.childID, func);
                },
                once: function(channel, func){
                    return t.realIpc.once(ipcMessagePrefix+channel[0].toUpperCase()+channel.substring(1)+t.childID, func);
                }, 
...
//                }
        ...
        resize(width, height){
            if(!this.fullScreen){                
                ...
                this.window.setSize(Math.ceil(width), Math.ceil(height));
                this.__onResize(width, height, oldSize[0], oldSize[1]);
            }
        }
        minimize(){
