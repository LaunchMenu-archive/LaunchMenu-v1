var EventHandler = (function(){
    ...
    var getTypes = function(obj, first){
        var maxLength = 8;
    	 if(obj instanceof Array){
    		...
    		if(obj.className || (keys.length>maxLength && !first)){
    ...
    eh.trigger = function(type, caller, data){
        if(!eventsSuppressed){
            ...
            {
                var typeName = type;
                if(caller.className) typeName = caller.className+"."+type;
            ...
            if(caller.className){ //caller is made using Class.js
                var classes = [{className:caller.className}];
                if(caller.parentList) classes = classes.concat(caller.parentList);
                
                listenerIterator:
                for(var n=0; n<classes.length; n++){
                    var c = classes[n].className;
                    
                    var l = listeners[c+"."+type[0]];
