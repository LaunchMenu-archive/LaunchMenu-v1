    ...
    function setupScrollbar(element, args){
        ...
        var observer = new MutationObserver(function(mutations){
            mutations.forEach(function(mutation) {
                var index = element.children("[scrollPane]").children("[scrollContent]").index();
                if(index==-1){
                    observer.disconnect();
                    element.text("");
                ...
                if(divs.length>0){
                    ...
                    el.updateSize();
                    if(el.setVerticalBar)
                        el.setVerticalBar(el.getVerticalBar()); //update bar offset and fade
                    if(el.setHorizontalBar)
                        el.setHorizontalBar(el.getHorizontalBar()); //update bar offset and fade
                }
            });    
        });
        var config = {childList: true, characterData: true, sub$Tree:args.detectDescendantAppend};
        ...
        if(args.vertical){
            element.append("<div class=se scrollbar v><div class='se bg8' bar></div></div>");
        ...
        if(el.setVerticalBar===undefined){
            ...
            if(args.vertical){
                ...
                var curVerticalFocusAn;
                el.focusVertical = function(offset, duration){
                    if(duration==null) duration = args.focusDuration;
                    ...
                    if(curVerticalFocusAn) curVerticalFocusAn.stop(true, true);
                    curVerticalFocusAn = $({offset:curOffset});
                    curVerticalFocusAn.animate({offset:offset}, {duration:duration, step:function(val){
            ...
            if(args.horizontal){
                ...
                el.focusHorizontal = function(offset, duration){
                    ...
                    if(curHorizontalFocusAn) curHorizontalFocusAn.stop(true, true);
                    curHorizontalFocusAn = $({offset:curOffset});
                    curHorizontalFocusAn.animate({offset:offset}, {duration:duration, step:function(val){
            ...
            el.updateSize = function(firstTime){
                var content = element.children("[scrollPane]").children("[scrollContent]");
                var widthPer = Math.min(1, element.width()/content.width());
                var heightPer = Math.min(1, element.height()/content.height());
                ...
                if(args.hideIfNoOverflow){
                    ...
                    if(args.vertical)
                        if(heightPer==1){
                            if(el.isVerticalScrollbarShown()) el.hideVerticalScrollbar(0); //hide instantly if it is the first time updateSize() is called (the initialisation of the element)
                        }else
                            if(!el.isVerticalScrollbarShown() && !args.autoHide) el.showVerticalScrollbar(0);
                    if(args.horizontal)
                        if(widthPer==1){
                            if(el.isHorizontalScrollbarShown()) el.hideHorizontalScrollbar(0);
                        }else
                            if(!el.isHorizontalScrollbarShown() && !args.autoHide) el.showHorizontalScrollbar(0);
                }
            }
        }
