    ...
    $Querier.acronymMatch = {
        ...
        getRegex:function(query, caseSensitive){
            ...
            for(var i=0;i<upperQuery.length;i++){
                var upper = $Utils.regexEscape(upperQuery[i]);
                var lower = $Utils.regexEscape(lowerQuery[i]);
                if(i==0){
            ...
            for(var i=1; i<parts.length; i++){
                ...
                if(!caseSensitive){ //case insensitive
                    ...
                    for(var i2=0; i2<upper.length; i2++)
                        m+="["+$Utils.regexEscape(lower[i2])+$Utils.regexEscape(upper[i2])+"]";
                }else
                    m = $Utils.regexEscape(part[i]);
                regex += "("+m+")([^A-Z]*)";
            }
            regex+="$";
    ...
    $Querier.characterMatch = {
        ...
        getRegex: function(query, caseSensitive){
            ...
            for(var i=0;i<lowerQuery.length;i++){
                var m;
                if(!caseSensitive){ //case insensitive
                    m = $Utils.regexEscape(lowerQuery[i])+$Utils.regexEscape(upperQuery[i]);
                }else
                    m = $Utils.regexEscape(query[i]);
                regex += (i==0?"(.*)":"([^"+m+"]*)")+"(["+m+"])";
            }
            regex +="(.*)$";
            return new RegExp(regex);
...
//
//function $Utils.regexEscape(str){ //is included in utilities, but needs to be here for worker to work
...
//        var escChar = $Utils.regexEscape(lowerQuery[i]);
...
//        var word = $Utils.regexEscape(queryWords[i]);
//        var insensitiveWord = "";
//        if(!casesensitive)
...
//        var escChar = $Utils.regexEscape(query[i]);
